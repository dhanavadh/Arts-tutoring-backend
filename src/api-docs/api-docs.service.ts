...
import {
  ...
  ApiController,
  ApiSpecification,
} from './interfaces/api-spec.interface';
...
export class ApiDocsService {
  ...
  async getApiSpecification(): Promise<ApiSpecification> {
    ...
    console.log(
      ...
      controllerWrappers.map((w) => w.metatype?.name),
    );
    ...
    for (const wrapper of controllerWrappers) {
      ...
      if (
        controllerName.includes('Internal') ||
        controllerName.includes('HealthCheck')
      ) {
        continue;
      }
      ...
      console.log('Controller:', controllerName, 'Methods:', methodNames);
      ...
      for (const methodName of methodNames) {
        ...
        if (!httpMethod || typeof httpMethod !== 'string') continue;

        // Get roles and guards
        let roles = this.reflector.get('roles', methodRef);
        if (!Array.isArray(roles)) {
          roles = roles !== undefined && roles !== null ? [roles] : [];
        }
        const guards =
          this.reflector.getAll('__guards__', [methodRef, metatype]) || [];
  ...
  private generateEndpointDescription(endpoint: ApiEndpoint): string {
    ...
    if (handler.startsWith('create'))
      return `Create a new ${this.extractResource(controller)}`;
    if (handler.startsWith('get') && handler.includes('All'))
      return `Get all ${this.extractResource(controller, true)}`;
    if (handler.startsWith('get') && handler.includes('ById'))
      return `Get ${this.extractResource(controller)} by ID`;
    if (handler.startsWith('get'))
      return `Get ${this.extractResource(controller)} information`;
    if (handler.startsWith('update'))
      return `Update ${this.extractResource(controller)}`;
    if (handler.startsWith('delete'))
      return `Delete ${this.extractResource(controller)}`;
    if (handler.startsWith('assign'))
      return `Assign ${this.extractResource(controller)}`;
    if (handler.startsWith('submit'))
      return `Submit ${this.extractResource(controller)}`;
    if (handler.startsWith('publish'))
      return `Publish ${this.extractResource(controller)}`;
    if (handler.startsWith('unpublish'))
      return `Unpublish ${this.extractResource(controller)}`;
